...
// Creation of mel filterbank (Slaney-style, librosa default)
void MFCCExtractor::createMelFilterBank()
{
    // Librosa defaults: fmin=0, fmax=sr/2, htk=False.
    float low_freq = 0.0f;
    ...
    for (int m = 1; m <= NUM_MEL_FILTERS; m++)
    {
        ...
        // librosa default filter normalization (mel_norm='slaney')
        float denom_hz = hz_points[m + 1] - hz_points[m - 1];
        if (denom_hz > 1e-9f)
        {
            float enorm = 2.0f / denom_hz;
            for (int k = 0; k < half_fft; k++)
            {
                mel_filters[(m - 1) * half_fft + k] *= enorm;
            }
        }
    }
...
{
    // librosa.feature.mfcc path has no explicit pre-emphasis.
    // Keep direct Hann-windowed framing to match training pipeline.
    for (int i = 0; i < N_FFT; i++)
    {
        _processed_frame[i] = frame[i] * window[i];
    }
...
{
    // Match librosa's centered framing behavior by zero-padding at boundaries.
    const int frame_center_offset = N_FFT / 2;

    for (int frame = 0; frame < NUM_FRAMES; frame++)
    {
        int start_sample = frame * HOP_LENGTH - frame_center_offset;

        for (int i = 0; i < N_FFT; i++)
        {
            int src_index = start_sample + i;
            if (src_index < 0 || src_index >= ANALYSIS_SAMPLES)
            {
                _frame_data[i] = 0.0f;
            }
            else
            {
                _frame_data[i] = audio[src_index];
            }
        }
...
bool AudioProcessor::begin(int sampleRate)
{
    ...
    // Start the I2S peripheral (mirrors working baseline sketch)
...
float AudioProcessor::convertI2SToFloat(int32_t i2s_sample)
{
    ...
    if (!_i2s_format_locked)
    {
        ...
        {
            _i2s_left_justified_24bit = (_i2s_probe_lsb_zero_count * 10 >= _i2s_probe_count * 9);
            _i2s_format_locked = true;
            Serial.printf("I2S sample format: %s-justified 24-bit\n",
                          _i2s_left_justified_24bit ? "left" : "right");
        }
    }

    int32_t audio_24bit = 0;

    if (_i2s_left_justified_24bit)
    {
        // Left-justified 24-bit payload in bits [31:8]
        audio_24bit = i2s_sample >> 8;
    }
    else
    {
        // Right-justified 24-bit payload in bits [23:0]
    ...
    float sample = (float)audio_24bit / 8388608.0f;
...
int AudioProcessor::read(int32_t *buffer, int bufferLength)
{
    ...
    {
        Serial.println("Audio buffer not set in AudioProcessor");
        return 0;
    }
